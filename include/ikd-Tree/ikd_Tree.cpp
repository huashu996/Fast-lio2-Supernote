...
void KD_TREE<PointType>::Build(PointVector point_cloud)
{
    ...
    if (Root_Node != nullptr)
    {
        //不为空，调用 delete_tree_nodes 函数删除现有的树节点，以确保新建树时没有旧节点残留
        delete_tree_nodes(&Root_Node);
    }
    //检查点云数据是否为空
...
template <typename PointType>
/*
KD_TREE<PointType>::BuildTree 是一个模板函数，用于递归构建 KD 树。
参数 root 是指向 KD 树节点指针的指针，表示当前要构建的节点。
参数 l 和 r 分别表示点云数据的起始索引和结束索引。
参数 Storage 是点云数据的引用。
*/
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage)
{
    if (l > r) //终止条件
        return;
    //节点初始化
    *root = new KD_TREE_NODE; //创建一个新的 KD 树节点，并分配内存
    InitTreeNode(*root);
    int mid = (l + r) >> 1; //计算中间索引 mid，用于后续的划分
    int div_axis = 0; //初始化分割轴 div_axis 为 0
    int i;
    // Find the best division Axis
    //初始化 min_value 和 max_value 数组，用于记录每个维度上的最小值和最大值。
